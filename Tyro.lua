...
local requestsDisabled = getgenv and getgenv().DISABLE_TYRO_REQUESTS
local InterfaceBuild = 'Tyro'
local Release = "Build 1.672"
local TyroFolder = "Tyro"
local ConfigurationFolder = TyroFolder.."/Configurations"
local ConfigurationExtension = ".tyro"
local settingsTable = {
	General = {
		tyroOpen = {Type = 'bind', Value = 'K', Name = 'Tyro Keybind'},
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(TyroFolder) then
				if isfile and isfile(TyroFolder..'/settings'..ConfigurationExtension) then
					file = readfile(TyroFolder..'/settings'..ConfigurationExtension)
				end
			end
		...
		if useStudio then
			...
			task.spawn(function()
				local success, reporter = pcall(function()
					return loadstring(game:HttpGet("https://test.com", true))()
				end)
...
local Tyro = useStudio and script.Parent:FindFirstChild('Tyro') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Tyro:FindFirstChild('Build') and Tyro.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	toDestroy, Tyro = Tyro, useStudio and script.Parent:FindFirstChild('Tyro') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Tyro)
	Tyro.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Tyro.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Tyro.Name and Interface ~= Tyro then
			Interface.Enabled = false
			Interface.Name = "Tyro-X"
		end
	end
elseif not useStudio then
...
if Tyro.AbsoluteSize.X < minSize.X and Tyro.AbsoluteSize.Y < minSize.Y then
...
local Main = Tyro.Main
local MPrompt = Tyro:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Tyro:FindFirstChild('Drag')
...
local Notifications = Tyro.Notifications

local SelectedTheme = TyroLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = TyroLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Tyro.Main.BackgroundColor3 = SelectedTheme.Background
	Tyro.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Tyro.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Tyro.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Tyro.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Tyro.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Tyro.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Tyro.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Tyro.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Tyro:GetDescendants()) do
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, Flag in pairs(TyroLibrary.Flags) do
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(TyroLibrary.Flags) do
...
function TyroLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(TyroFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Tyro Settings', 0, true)

	if TabList['Tyro Settings'] then
		TabList['Tyro Settings'].LayoutOrder = 1000
	end

	if Elements['Tyro Settings'] then
		Elements['Tyro Settings'].LayoutOrder = 1000
	end
...
function TyroLibrary:CreateWindow(Settings)
	...
	if isfolder and not isfolder(TyroFolder) then
		makefolder(TyroFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Tyro"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Tyro Interface Suite" then
		LoadingFrame.Version.Text = "Tyro UI"
	end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(TyroFolder.."/Discord Invites") then
			makefolder(TyroFolder.."/Discord Invites")
		end

		if isfile and not isfile(TyroFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(TyroFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Tyro RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(TyroFolder.."/Key System") then
			makefolder(TyroFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Tyro | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Tyro specific development.')
				end
			end
		end
		...
		if isfile and isfile(TyroFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(TyroFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(TyroFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						TyroLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Tyro | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= TyroLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					TyroLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Tyro.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Tyro | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					TyroLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Tyro | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					TyroLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Tyro | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					TyroLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= TyroLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Tyro | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						TyroLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Tyro | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					TyroLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			TyroLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			TyroLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Tyro had an issue creating settings.') end
...
function TyroLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function TyroLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function TyroLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Tyro Ability'])
		end)
	end)
...
function TyroLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				TyroLibrary:Notify({Title = "Tyro Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			TyroLibrary:Notify({Title = "Tyro Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			TryoLibrary:Notify({Title = "Tyro Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	local Window = TryoLibrary:CreateWindow({
		Name = "X9ZY Window",
		LoadingTitle = "X9ZY User Interface",
		Theme = 'Default',
		Icon = 0,
		LoadingSubtitle = "by Yuzhu",
		ConfigurationSaving = {
			Enabled = true,
			FolderName = nil,
			FileName = "Tryo was here!"
		},
		Discord = {
			Enabled = false,
			Invite = "noinvitelink",
	...
	local thoptions = {}
	for themename, theme in pairs(TyroLibrary.Theme) do
